...
# Prometheus section (defaults):
# prometheus.path = /metrics
# prometheus.tcp.ip = 0.0.0.0

# Peer Discovery Plugin section
      cluster_formation.peer_discovery_backend  = rabbit_peer_discovery_k8s
      cluster_formation.k8s.host = {{ .Values.kubeApi }}
      ...
      cluster_formation.k8s.service_name = {{ .Values.global.name }}-internal-service
      cluster_formation.k8s.hostname_suffix = .{{ .Values.global.name }}-internal-service.{{ .Values.global.name }}-ns.svc.cluster.local
...
default_vhost = /
default_user = admin
default_pass = piedpiper_top_secret_pass
...
##
management.http_log_dir = /var/log/rabbitmq/http_api
...
## File rotation config. No rotation by default. in BITES
## DO NOT SET rotation date to ''. Leave the value unset if "" is the desired value
# log.file.rotation.date = $D0

# in BITES 100MB=104857600
...
## to work properly.
##
 auth_ldap.servers.1 = vlg-dc01.megafon.ru
...
## binding
##
auth_ldap.user_dn_pattern = ${username}@megafon.ru
...
auth_ldap.dn_lookup_base      = dc=Megafon,dc=ru
